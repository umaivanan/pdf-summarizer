...
app.use(
  fileUpload({
    tempFileDir: '/tmp', // Use Vercel's /tmp directory
    useTempFiles: true,
    debug: true,
  })
);
...
app.post('/summary', async function (req, res) {
  ...
  // Set the file upload path to the /tmp directory provided by Vercel
  uploadPath = path.join('/tmp', sampleFile.name); // Store temporarily in the /tmp folder
  ...
  sampleFile.mv(uploadPath, async function (err) {
    ...
    try {
      ...
      // Clean up the file after processing (Vercel allows cleanup within /tmp)
      fs.unlinkSync(uploadPath);
